...
import { L as Link, Q as RouterContextProvider, Sn as useRevalidator, dn as useLocation, hn as useNavigation, in as useFetchers, l as createRequestHandler$1, mn as useNavigate, mt as createContext$2, pn as useMatches, rn as useFetcher } from "./chunk-2UH5WJXA-CUBj4kRL.js";
...
function createStorefrontClient(options) {
	...
	if (!cache) warnOnce(H2_PREFIX_WARN + "Storefront API client created without a cache instance. This may slow down your sub-requests.");
...
async function hydrogenRoutes(currentRoutes) {
	const { getVirtualRoutesV3 } = await import("./get-virtual-routes-ACFZHU42-eeinoit0.js");
...
function hydrogenPreset() {
	return {
		...
		reactRouterConfigResolved: ({ reactRouterConfig }) => {
			...
			if (reactRouterConfig.future?.unstable_subResourceIntegrity === true) throw new Error("[Hydrogen Preset] unstable_subResourceIntegrity cannot be enabled.\nReason: Conflicts with Hydrogen CSP nonce-based authentication.\nImpact: Would break Content Security Policy and cause script execution failures.");
		}
	};
}
...
async function storefrontRedirect(options) {
	...
	if (url.pathname === "/admin" && !noAdminRedirect) return createRedirectResponse(`${storefront.getShopifyDomain()}/admin`, isSoftNavigation, searchParams, matchQueryParams);
	try {
...
var SeoLogger = (0, import_react.lazy)(() => import("./log-seo-tags-IG37ONQ2-aKFFGR4h.js"));
function Seo({ debug }) {
