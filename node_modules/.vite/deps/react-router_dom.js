...
import { $ as RouterProvider, At as getPatchRoutesOnNavigationFunction, Ft as invariant, It as isMutationMethod, J as RemixErrorBoundary, Jt as setIsHydrated, Mt as getTurboStreamSingleFetchDataStrategy, Nt as hydrationRouteProperties, O as ErrorResponseImpl, Ot as decodeViaTurboStream, Pt as invalidProtocols, Rt as mapRouteProperties, T as CRITICAL_CSS_DATA_ATTRIBUTE, Vt as noActionDefinedError, Xt as singleFetchUrl, Yt as shouldHydrateRouteLoader, Zt as stripIndexParam, an as useFogOFWarDiscovery, bt as createRequestInit, ft as createClientRoutes, j as FrameworkContext, jt as getSingleFetchDataStrategyImpl, m as getHydrationData, mt as createContext, n as RSCRouterGlobalErrorBoundary, p as deserializeErrors, pt as createClientRoutesWithHMRRevalidationOptOut, q as RSCRouterContext, ut as createBrowserHistory, v as populateRSCRouteModules, xt as createRouter } from "./chunk-2UH5WJXA-CUBj4kRL.js";
import { t as require_react_dom } from "./react-dom-DAJI4n8j.js";
...
function createCallServer({ createFromReadableStream, createTemporaryReferenceSet, encodeReply, fetch: fetchImplementation = fetch }) {
	...
	return async (id, args) => {
		...
		import_react.startTransition(() => Promise.resolve(payloadPromise).then(async (payload) => {
...
function RSCHydratedRouter({ createFromReadableStream, fetch: fetchImplementation = fetch, payload, getContext }) {
	...
	let { router: router2, routeModules } = import_react.useMemo(() => createRouterFromPayload({
		payload,
		fetchImplementation,
		getContext,
	...
	let [{ routes, state }, setState] = import_react.useState(() => ({
		routes: cloneRoutes(router2.routes),
		state: router2.state
	}));
	import_react.useLayoutEffect(() => router2.subscribe((newState) => {
		if (diffRoutes(router2.routes, routes)) import_react.startTransition(() => {
			setState({
				routes: cloneRoutes(router2.routes),
				state: newState
			});
	...
	const transitionEnabledRouter = import_react.useMemo(() => ({
		...router2,
		state,
		routes
	}), [
